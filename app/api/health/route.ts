...
export async function GET(request: Request) {
  ...
  const checks = [
    ...
    { key: "audit", label: "Persistent audit store", ready: false, weight: 10 },
  ...
  return NextResponse.json({
    ...
    checks,
    services: { api: "online", model: "online", rag: process.env.OPENAI_API_KEY ? "semantic" : "keyword", auth: authConfigured() ? "protected" : "unconfigured" },
