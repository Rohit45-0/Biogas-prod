...
export default function Home(){
  ...
  const predict=async()=>{ setLoading(true); setResult(null); await new Promise(r=>setTimeout(r,1050));
    ...
    const data=await res.json(); setResult(data); setLastRunInputs({...inputs}); setHistory(h=>[{time:new Date().toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"}),biogas:data.biogas,electricity:data.electricity},...h].slice(0,4)); setLoading(false);
  };
