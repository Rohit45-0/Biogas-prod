...
function OperationsDashboard({auth,onLogout,onSettings}:{auth:AuthUser;onLogout:()=>void;onSettings?:()=>void}){
  ...
  const kpis=[
    ["BIOGAS",daily?.optimizedBiogasM3Day,"m³/day","AI predicted","♨"],
    ["CH₄",methanePct,"% methane","AI predicted","◇"],
    ["CH₄ / DAY",daily?.optimizedMethaneM3Day,"m³ CH₄/day","AI predicted","♨"],
    ["ELECTRICITY",daily?.optimizedElectricityKwhDay,"kWh/day","AI predicted","ϟ"],
    ["AI GAIN",extraBiogas,"m³ biogas/day","vs baseline","↗"],
  ] as const;
  ...
  return <main className="ops-shell">
    ...
    <section className="ops-main">
      ...
      {tab==="overview"&&<section className="ops-view ops-overview">
        <div className="ops-kpi-grid">{kpis.map(([label,value,unit,status,icon],index)=><article key={label} className={`ops-kpi tone-${index}`}><span className="ops-kpi-icon">{icon}</span><div><small>{label}</small><b>{value===undefined?"—":`${index===4&&value>=0?"+":""}${format(value)}`}</b><em>{value===undefined?"Run AI model":unit}</em><span><i/>{status}{daily&&index<4?<u>Baseline {format(index===0?daily.baselineBiogasM3Day:index===1?(daily.baselineMethaneM3Day/daily.baselineBiogasM3Day*100):index===2?daily.baselineMethaneM3Day:daily.baselineElectricityKwhDay)}</u>:null}</span></div></article>)}</div>
        ...
        <div className="ops-mid-grid">
          ...
          <section className="ops-optimizer-card"><header><b>AI Optimizer</b><button onClick={()=>setTab("optimizer")}>Open details →</button></header><div className="ops-mini-table"><div className="head"><span>Parameter</span><span>Current</span><span>AI recommended</span></div>{adjustments.slice(0,5).map(([label,current,target,unit])=><div key={label}><b>{label}</b><span>{format(current)} {unit}</span><strong>{format(target)} {unit} <i>{target>current?"↑":target<current?"↓":"→"}</i></strong></div>)}{!best&&<p>Run the model to calculate recommended setpoints.</p>}</div></section>
      ...
      {tab==="explorer"&&<section className="ops-view ops-explorer-view"><div className="ops-view-heading"><div><small>DATA EXPLORER</small><h2>Calculated AI scenario evidence</h2><p>Inspect the highest-ranked rows returned by the current server report.</p></div>{report&&<a href={`/api/reports/batch?id=${report.id}&format=csv`}>Export all 2,000 rows ↓</a>}</div>{report?<div className="ops-explorer-table"><header><span>Rank</span><span>Source</span><span>HRT</span><span>Baseline biogas</span><span>AI biogas</span><span>AI methane</span><span>AI electricity</span></header>{report.preview.slice(0,12).map(row=><div key={row.scenario_id}><b>#{row.rank}</b><span>{row.source_run_id||row.scenario_id}</span><span>{format(row.hrt_hours)} h</span><span>{format(row.baseline_biogas_m3_day)} m³/d</span><strong>{format(row.optimized_biogas_m3_day)} m³/d</strong><span>{format(row.methane_m3_day)} m³/d</span><span>{format(row.electricity_kwh_day)} kWh/d</span></div>)}{report.preview.length===0&&<p>The saved summary is loaded. Generate a fresh report to inspect its top scenario rows in this table.</p>}</div>:<div className="ops-empty"><span>⌕</span><h3>No scenario evidence yet</h3><p>Run the model to calculate the scenario table.</p><button onClick={()=>void generate()}>Run AI model</button></div>}</section>}
