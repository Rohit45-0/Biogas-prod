...
type Prediction = { biogas:number; methanePct:number; methane:number; electricity:number; carbon:number; codRemoval:number; stability:number; confidence:number; improvement:number; pressure:number; h2s:number; recommendations:{title:string;detail:string;impact:number;tone:string}[]; forecast:number[] };

const initial: Inputs = { feedstock:"Dairy wastewater", feedRate:820, temperature:35.2, ph:6.82, olr:2.2, hrt:22, codIn:7000, vfa:1100, mixing:46 };
...
const quickQuestions = ["How can I increase methane?","Explain this prediction","Is my pH safe?"];
...
export default function Home(){
  ...
  const predict=async()=>{ setLoading(true); setResult(null); await new Promise(r=>setTimeout(r,1050));
    ...
    const data=await res.json(); setResult(data); setHistory(h=>[{time:new Date().toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"}),biogas:data.biogas,electricity:data.electricity},...h].slice(0,4)); setLoading(false);
  };
  ...
  return <main className="shell">
    ...
    <section className="workspace">
      ...
      <section className="kpis">
        ...
        <Kpi label="Model Confidence" value={`${shown.confidence.toFixed(0)}%`} color="#6d28d9" note="Linear model"/>
      ...
      <section className="main-grid">
        <div className="card input-card"><Title title="PLANT INPUT & DIGITAL TWIN" badge="MANUAL MODE"/>
          <div className="input-layout"><div className="form-grid">
            ...
            <Field label="Feed rate" value={inputs.feedRate} unit="kg VS/d" onChange={v=>update("feedRate",v)}/>
            <Field label="Temperature" value={inputs.temperature} unit="°C" onChange={v=>update("temperature",v)}/>
            <Field label="pH level" value={inputs.ph} onChange={v=>update("ph",v)} step="0.01"/>
            <Field label="Organic loading" value={inputs.olr} unit="kg VS/m³·d" onChange={v=>update("olr",v)} step="0.1"/>
            <Field label="Retention time" value={inputs.hrt} unit="days" onChange={v=>update("hrt",v)}/>
        ...
        <div className="card optimization"><Title title="AI OPTIMIZATION ENGINE" badge={result?"NEW ANALYSIS":"AWAITING INPUT"}/>
          ...
          </div><h3 className="rec-title">AI agent recommendations</h3><div className="recommendations">{result.recommendations.map((r,i)=><div className="recommendation" key={i}><span className={r.tone}>↗</span><div><b>{r.title}</b><small>{r.detail}</small></div><em>+{r.impact.toFixed(1)}%</em></div>)}</div></>:<div className="empty"><span>✦</span><b>Ready to optimize</b><p>Enter current plant conditions and run a prediction. The agent will calculate outputs and suggest safe adjustments.</p></div>}
        </div>
      </section>
      ...
      <footer><span><i></i> Cloud API online</span><span><i></i> Model service online</span><span><i></i> Knowledge base: 4 workbooks</span><small>Prototype outputs are estimates from synthetic/demo data.</small></footer>
    </section>

    {chatOpen&&<div className="chat"><div className="chat-head"><div><span>✦</span><b>Aqua Copilot</b><small>Data-aware assistant</small></div><button onClick={()=>setChatOpen(false)}>×</button></div><div className="messages">{messages.map((m,i)=><div key={i} className={`message ${m.role}`}>{m.text}</div>)}{chatBusy&&<div className="message ai typing"><i></i><i></i><i></i></div>}</div><div className="quick">{quickQuestions.map(q=><button onClick={()=>ask(undefined,q)} key={q}>{q}</button>)}</div><form onSubmit={ask}><input value={question} onChange={e=>setQuestion(e.target.value)} placeholder="Ask about your plant data…"/><button>➤</button></form></div>}
  </main>
}

function Title({title,badge}:{title:string;badge:string}){return <div className="card-title"><h2>{title}</h2><span>{badge}</span></div>}
function Kpi({label,value,color,note}:{label:string;value:string;color:string;note:string}){return <div className="kpi card"><div className="ring" style={{"--color":color} as React.CSSProperties}><b>{value}</b></div><div><span>{label}</span><small>{note}</small><i style={{background:color}}></i></div></div>}
function Field({label,value,unit,onChange,options,step}:{label:string;value:string|number;unit?:string;onChange:(v:string)=>void;options?:string[];step?:string}){return <label className="field"><span>{label}</span><div>{options?<select value={value} onChange={e=>onChange(e.target.value)}>{options.map(o=><option key={o}>{o}</option>)}</select>:<input type="number" step={step||"1"} value={value} onChange={e=>onChange(e.target.value)}/>} {unit&&<em>{unit}</em>}</div></label>}
