  ...
  return <main className="simple-shell">
    <aside className="simple-sidebar">
      <nav>
        ...
        <button onClick={()=>openOverviewSection("inputs")}><span>▦</span>Plant readjustment</button>
        <button onClick={()=>openOverviewSection("ai-kpi-report")}><span>▤</span>AI & KPI report</button>
    ...
    <section className="simple-workspace" id="overview">
      ...
      {workspaceView==="batch"&&<BatchReportsPage onOverview={()=>openOverviewSection()}/>}
      {workspaceView==="kpi"&&<KpiReportsPage onOverview={()=>openOverviewSection()}/>}
      {workspaceView!=="overview"&&workspaceView!=="audit"&&workspaceView!=="batch"&&workspaceView!=="kpi"&&<FocusedOutputPage
      ...
      <header className="simple-header">
        <div><p>AI WASTEWATER-TO-ENERGY COMMAND CENTER</p><h1>AQUAIVOLT</h1><span>Wastewater to Optimized Energy</span></div>
        <div className="header-tools"><span className="system-ready"><i/>System ready</span><button onClick={()=>setChatOpen(true)}>✦ Ask Copilot</button><div className="user-chip"><b>{auth.username.slice(0,2).toUpperCase()}</b><span>{auth.username}<small>{auth.role}</small></span></div><button className="header-logout" type="button" onClick={()=>void logout()} aria-label="Log out">Log out</button></div>
      </header>

      <div className="truth-strip"><b>2–24 hour HRT model</b><span>Plant readjustment and online reading are two ways to run the trained model. Simulated real-time readings are shown until equipment is connected.</span></div>
      <DataSourceRail active="manual"/>

      <section className="input-panel" id="inputs">
        <div className="section-heading"><div><small>STEP 1</small><h2>Enter the current digester condition</h2><p>All five fields below are used by the trained 2–24 hour HRT model.</p></div><span className="inside-range">Readjustment only</span></div>
        <div className="input-grid primary-inputs">
          ...
          <Field label="Organic loading" value={inputs.olr} unit="kg VS/m³·d" onChange={value=>update("olr",value)} step="0.01"/>
          <Field label="Retention time (HRT)" value={inputs.hrt} unit="hours" onChange={value=>update("hrt",value)} step="0.1"/>
        </div>
        {dirty&&<div className="dirty-note">Plant readjustment values changed. Run the calculation again to refresh every result.</div>}
        {predictionError&&<div className="input-warning">{predictionError}</div>}
        <div className="extrapolation-note">Supported ranges: feed 865.8–929.4 kg VS/day, temperature 37–65.2 °C, pH 6.19–6.93, OLR 4.43–70.4 and HRT 2–24 hours. Nothing runs until you click Calculate.</div>
        <button className="calculate-button" disabled={loading} onClick={()=>void predict()}>{loading?<><i className="loader"/>AI is comparing plant values…</>:"Calculate production →"}</button>
      </section>

      <BatchReportsPage embedded onOverview={()=>openOverviewSection()}/>

      <section className="history-panel" id="history">
        <div className="section-heading compact"><div><small>STEP 2</small><h2>Compare recent calculations</h2><p>Each row uses the plant values applied for that run.</p></div></div>
        <RunTable runs={runs} outputKey={activeOutput}/>
      </section>

      <section className="virtual-monitor-panel" id="virtual-monitoring" aria-live="polite">
        <div className="section-heading"><div><small>STEP 2A • VIRTUAL PLANT</small><h2>Monitoring system</h2><p>Latest plant readjustment result shown as a simulated online reading.</p></div><span className="simulation-label">SIMULATED ONLINE READING</span></div>
      ...
      <section className="results-section" aria-live="polite">
        <div className="section-heading"><div><small>STEP 3</small><h2>See the result</h2><p>Only real quantities: before, after and the extra amount.</p></div>{result&&<span className="run-pill">Run {result.runId.split("-")[0].toUpperCase()}</span>}</div>
      ...
      <section className="production-detail-panel" id="production-detail" aria-live="polite">
        ...
        {loading?<Working/>:result&&lastRunInputs?<ProductionDetail outputKey={activeOutput} result={result} inputs={lastRunInputs}/>:<div className="detail-empty"><span>◎</span><b>Production details are waiting</b><p>Run Calculate production to see the modeled total, best conditions and values that need attention.</p></div>}
      </section>
