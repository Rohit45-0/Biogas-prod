...
function OperationsDashboard({auth,onLogout,onSettings}:{auth:AuthUser;onLogout:()=>void;onSettings?:()=>void}){
  ...
  const kpis:{label:string;value:number|undefined;unit:string;status:string;icon:string;tone:"blue"|"green"|"amber";positive?:boolean}[]=[
    {label:"BASELINE BIOGAS",value:daily?.baselineBiogasM3Day,unit:"m³/day",status:"Modelled without AI",icon:"◓",tone:"blue"},
    {label:"AI BIOGAS",value:daily?.optimizedBiogasM3Day,unit:"m³/day",status:"Best ranked output",icon:"♨",tone:"green"},
    {label:"METHANE",value:daily?.optimizedMethaneM3Day,unit:"m³ CH₄/day",status:methanePct===undefined?"Run AI model":`${format(methanePct)}% of biogas`,icon:"◆",tone:"green"},
    {label:"ELECTRICITY",value:daily?.optimizedElectricityKwhDay,unit:"kWh/day",status:"AI optimized",icon:"ϟ",tone:"blue"},
    {label:"EXTRA BIOGAS",value:extraBiogas,unit:"m³/day",status:"Above baseline",icon:"↗",tone:"green",positive:true},
    {label:"H₂S REMOVED",value:daily?.h2sRemovedPpm,unit:"ppm",status:"Derived estimate",icon:"◉",tone:"amber"},
    {label:"CO₂e AVOIDED",value:daily?.estimatedCo2eAvoidedKgDay,unit:"kg/day",status:"Derived estimate",icon:"♧",tone:"green"},
    {label:"AI SCENARIOS",value:report?.summary.totalRows,unit:"model outputs",status:report?.persisted?"Saved in reports":"Available in reports",icon:"▦",tone:"blue"},
  ];
