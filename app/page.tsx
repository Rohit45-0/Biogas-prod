...
type PlantOutput = { gasFlow:number; biogas:number; methanePct:number; methane:number; electricity:number; generatorKw:number; carbon:number; co2Pct:number; h2s:number; oxygenPct:number };
...
type HourPoint = { hour:number; biogas:number; electricity:number; ch4:number; co2:number; h2s:number; o2:number };
...
type Prediction = {
  ...
  confidenceMeaning:string; baseline:PlantOutput; optimized:PlantOutput; gasComposition:GasMetric[]; alerts:Alert[]; equipmentStates:Equipment[];
...
type AdminSettings = { methaneMinimum:number; h2sWarning:number; oxygenMaximum:number; pressureMinimum:number; pressureMaximum:number; facilityName:string; facilityLocation:string };
    ...
    <section className="workspace" id="executive-overview">
      ...
      <section className="monitoring-grid" id="prediction-center">
        ...
        <div className="card gas-card">
          <Title title="GAS OPTIMIZATION" badge={result?"BEFORE vs AFTER":"AWAITING MODEL"}/>
          {result?<GasComparison metrics={result.gasComposition}/>:<AwaitingPrediction text="Four-gas comparisons will appear after prediction."/>}
        </div>
      </section>
...
function GasComparison({metrics}:{metrics:GasMetric[]}) {
  return <div className="gas-comparison"><div className="gas-legend"><span><i></i>Baseline</span><span><i></i>AI scenario</span></div>{metrics.map(metric=>{const max=Math.max(metric.before,metric.after)*1.15||1;const good=metric.direction==="up"?metric.after>=metric.before:metric.after<=metric.before;return <div className="gas-row" key={metric.key}><div><b>{metric.label}</b><small>{metric.name}</small></div><div className="gas-bars"><span style={{width:`${metric.before/max*100}%`}}></span><span style={{width:`${metric.after/max*100}%`}}></span></div><em className={good?"positive":"negative"}>{format(metric.before)} → {format(metric.after)} {metric.unit}</em></div>})}</div>;
}

function FacilityMap({result}:{result:Prediction|null}) {
  return <div className="facility-monitor"><div className="facility-map"><div className="map-road r1"></div><div className="map-road r2"></div><div className="map-road r3"></div><div className="plant-pin"><i></i><span>{result?.facility.name??"Aquaivolt plant"}<small>{result?.facility.location??"Location pending"}</small></span></div><b>FACILITY POSITION</b><small>Admin-configured coordinates will replace this simulation canvas.</small></div><div className="facility-stats"><div><span>Device availability</span><b>0 / 5 live</b><small>Hardware not connected</small></div><div><span>Simulated signals</span><b>{result?"9 inputs · 4 gases":"Awaiting run"}</b><small>Model data contract active</small></div><div><span>Alarm scenes</span><b>{result?result.alerts.filter(alert=>alert.status!=="normal").length:"—"}</b><small>Threshold-based simulation</small></div></div></div>;
}

function DeviceStatus({result}:{result:Prediction|null}) {
  const devices=[{name:"Gas analyzer · 4-in-1",signal:"CH₄ / CO₂ / H₂S / O₂"},{name:"Digester probe",signal:"Temperature / pH"},{name:"Flow transmitter",signal:"Gas flow / pressure"},{name:"Generator meter",signal:"kW / kWh"},{name:"PLC gateway",signal:"Valves / mixer / heating"}];
